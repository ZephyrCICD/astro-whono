...
const basePathSegment = String(
  process.env.ASTRO_WHONO_BASE_PATH ?? process.env.SITE_BASE ?? ''
).trim().replace(/^\/+|\/+$/g, '');
...
const appendBasePrefix = (siteUrl) => {
  if (!basePrefix) return siteUrl;

  const sitePathname = new URL(`${siteUrl}/`).pathname.replace(/\/+$/, '');
  return sitePathname === basePrefix || sitePathname.endsWith(basePrefix)
    ? siteUrl
    : `${siteUrl}${basePrefix}`;
};
...
export const runProductionArtifactCheck = async (options = {}) => {
  ...
  const publicSiteUrl = appendBasePrefix(siteUrl);
  ...
  expect(
    robotsTxt.includes(`Sitemap: ${publicSiteUrl}/sitemap-index.xml`),
    'robots.txt is missing the expected Sitemap line'
  );

  const sitemapXml = readText('dist/sitemap-0.xml');
  expect(
    sitemapXml.includes(`<loc>${publicSiteUrl}/about/</loc>`),
  ...
  expect(
    !sitemapXml.includes(`${publicSiteUrl}/bits/draft-dialog/`),
  ...
  expect(
    aboutHtml.includes(`<link rel="canonical" href="${publicSiteUrl}/about/"`),
    'About page canonical no longer matches SITE_URL'
  );
  expect(
    aboutHtml.includes(`<meta property="og:url" content="${publicSiteUrl}/about/"`),
  ...
  expect(
    sampleArchiveLink.startsWith(`${publicSiteUrl}/archive/`),
