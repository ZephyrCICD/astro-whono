...
const deploymentBaseEnvName = process.env.ASTRO_WHONO_BASE_PATH != null
  ? 'ASTRO_WHONO_BASE_PATH'
  : 'SITE_BASE';
const rawDeploymentBase = process.env.ASTRO_WHONO_BASE_PATH ?? process.env.SITE_BASE ?? '/';
...
// 规避：命令前加 MSYS_NO_PATHCONV=1（或将对应变量加入 MSYS2_ENV_CONV_EXCL）。
if (/[:\s]/.test(trimmedDeploymentBase)) {
  throw new Error(
    `Invalid ${deploymentBaseEnvName} "${rawDeploymentBase}": looks like a filesystem path, not a URL base. ` +
